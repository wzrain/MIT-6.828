...
sys_exofork(void)
{
	...
	struct Env *e;
	if (env_alloc(&e, curenv->env_id) < 0) return -E_NO_FREE_ENV;
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	if (ROUNDUP(srcva, PGSIZE) != srcva || ROUNDUP(dstva, PGSIZE) != dstva) return -E_INVAL;
	...
	pte_t* pte_store;
	struct PageInfo *pg = page_lookup(srcenv->env_pgdir, srcva, &pte_store);
...
syscall(uint32_t syscallno, uint32_t a1, uint32_t a2, uint32_t a3, uint32_t a4, uint32_t a5)
{
	...
	switch (syscallno) {
	case SYS_cputs: 
		sys_cputs((const char *) a1, a2); return 0;
	case SYS_cgetc:
		return sys_cgetc();
	case SYS_getenvid:
		return sys_getenvid();
	...
	return 0; /* MIGHT NOT COVER ALL CASES. */
}
